    ...
    for (var i = 0, l = nodes.length; i < l; i++) {
      ...
      if (nodeStack.indexOf(node) !== -1) {
        var _ret = (function () {
          var loop = []
            .concat(
              _toConsumableArray(nodeStack.slice(nodeStack.indexOf(node))),
              [node]
            )
            .map(function (d) {
      ...
      if (currentDepth > node.depth) {
        ...
        traverse(
          node.out,
          [].concat(_toConsumableArray(nodeStack), [node]),
      ...
      function paintLinks() {
        ...
        if (state.linkCanvasObject) {
          ...
          defaultPaintLinks = [].concat(
            _toConsumableArray(beforeCustomLinks),
            afterCustomLinks,
            otherCustomLinks
          );
        ...
        var linksPerColor = indexBy(defaultPaintLinks, [
          getColor,
          getWidth,
          getLineDash,
        ]);
              ...
              links.forEach(function (link) {
                ...
                if (!controlPoints) {
                  ...
                  ctx[
                    ...
                    ctx,
                    _toConsumableArray(controlPoints).concat([end.x, end.y])
                  );
                }
              });
  ...
  update: function update(state) {
    ...
    if (!state.isShadow) {
      ...
      state.graphData.links.forEach(function (link) {
        ...
        if (numPhotons) {
          link.__photons = _toConsumableArray(Array(numPhotons)).map(
            function () {
              return {};
            }
          );
  ...
  init: function init(domNode, state) {
    ...
    var getObjUnderPointer = function getObjUnderPointer() {
      ...
      var px =
        pointerPos.x > 0 && pointerPos.y > 0
          ...
          : null; // Lookup object per pixel color
    ...
    state.forceGraph
      ...
      .onFinishUpdate(function () {
        ...
        if (
          zoomTransform(state.canvas).k === state.lastSetZoom &&
